<?php
    ...
    if (isset($_GET['login'])) {
        if (isset($_GET['USEROREMAIL'])) {
            ...
            $query = "SELECT * FROM useres WHERE  (username='" . $_GET['USEROREMAIL'] . "' OR email='" . $_GET['USEROREMAIL'] . "') AND password='".$password."' ";
            $result = mysqli_query($connection, $query);

            if (mysqli_num_rows($result) == 1) {
                $info = mysqli_fetch_assoc($result);

                setcookie("userid", $info['userid'], time() + (30 * 60), "/");
                Header("Location: home.php");

            } else {

                array_push($errors, "There is a problem to login.");
            }
        ...
        <div class='content'>
            ...
            <form action="index.php" method="GET" >
                <div class='input-fields'>
                    ...
                    <input type="text" placeholder="username or email" name="USEROREMAIL" class='input-line full-width'>

                    <input type='password' placeholder='Password' class='input-line full-width' name="password">


                </div>
                <div class='spacing'>have an account? <a class='highlight' href="register.php">Register</a></div>
